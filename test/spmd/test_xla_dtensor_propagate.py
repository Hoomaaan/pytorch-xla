...
class XLADTensorPropagateTest(test_xla_sharding_base.XlaShardingTest):
  ...
  def test_propagate(self):
    ...
    try:
      ...
      if hasattr(spec_after, 'tile_assignment_dimensions'):
        print(
            f"tile_assignment_dimensions(): {spec_after.tile_assignment_dimensions()}"
        )
      if hasattr(spec_after, 'tile_assignment_devices'):
        print(
            f"tile_assignment_devices(): {spec_after.tile_assignment_devices()}"
        )
    ...
    xm.mark_step()
    try:
      spec_compiled = torch_xla._XLAC._get_xla_sharding_spec(
          sharded.global_tensor)
  ...
  def test_parse_xla_sharding_to_placements_replicated(self):
    ...
    placements = sharded_tensor._parse_xla_sharding_to_placements(
        '{devices=[1]0}')
